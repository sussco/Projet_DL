...
def testWithInterface(inFile, test_images, netScratch, netPyTorch):
    fenetre = Tk()
    ...
    for i in range(10):
        ...
        txt = canvas.create_text(1220, i*65 +110, text=switch_class(i), font="Arial 25", fill="white")

    txt = canvas.create_text(200, 40, text="Scratch", font="Arial 40 bold", fill="yellow")
    txt = canvas.create_text(1100, 40, text="PyTorch", font="Arial 40 bold", fill="yellow")

    for i in range(10):
        a2 = canvas.create_rectangle(130,i*65 + (65/2 - 5) +60,325, i*65 +3*65/2-25 + 60, outline = 'white')
        a2 = canvas.create_rectangle(970,i*65 + (65/2 - 5) +60,1165, i*65 +3*65/2-25 + 60, outline = 'white')
    ...
    for training in range(len(test_images[0])):
        ...
        resultatPyTorch = netScratch.propagation(test_images[0][training])
        for j in range(10):
            ...
            remplissage = canvas.create_rectangle(970,j*65 + 60 + (65/2 - 5), 970+195*resultatPyTorch[j], j*65 + 60 +3*65/2-25, fill ='white', outline = 'white')
            canvas.update()
        input()
        sScratch = np.argmax(resultatScratch)
        sPyTorch = np.argmax(resultatPyTorch)
        if sScratch == np.argmax(test_images[1][training]):
            colorScratch = 'green'
        else :
            colorScratch = 'red'
        if sPyTorch == np.argmax(test_images[1][training]):
            colorPyTorch = 'green'
        else :
            colorPyTorch = 'red'
        ...
        sortiePyTorch = canvas.create_rectangle(970,sPyTorch*65 + 60 + (65/2 - 5), 970+195*resultatPyTorch[sPyTorch], sPyTorch*65 + 60 +3*65/2-25, fill =colorPyTorch, outline = 'white')
        canvas.update()
        input()

        for i in range(10):
            a2 = canvas.create_rectangle(130,i*65 + (65/2 - 5) +60,325, i*65 +3*65/2-25 + 60, fill = 'black', outline = 'white')
            a2 = canvas.create_rectangle(970,i*65 + (65/2 - 5) +60,1165, i*65 +3*65/2-25 + 60, fill = 'black' ,outline = 'white')
        canvas.delete(image)
        canvas.delete(label)
        canvas.delete(sortieScratch)
        canvas.delete(sortiePyTorch)
...
def interfaceMnist(Perceptron, inFile, cas, rangeTest, test_images, labelled_images, rangeLabel):
    if cas == True :
        var = 25
    else :
        var = 65
...
testWithInterface('cifar-10-batches-py/test_batch', test_images, pickle.load(open('CIFAR10test', 'rb'))[0][1],pickle.load(open('CIFAR10test', 'rb'))[0][1])
